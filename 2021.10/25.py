# 1. 2805 - 나무 자르기 (https://www.acmicpc.net/problem/2805)
"""
문제 설명
...
while True :
    if m > res[mid] :
        ...
        print(stan - mid)
        break
    elif (end == mid or mid == start) and m != res[mid] :
        print(stan - mid + 1)
...
while left <= right :
    h = (left + right) // 2
    total = 0
    total = sum(i - h if i > h else 0 for i in li)
    if total < m :
        right = h - 1
    else :
        left = h + 1
...
"""
insight 정리
1. 이분 탐색을 할 때 꼭 인덱스를 기준으로 한다는 편견? 을 지양할 것.
-> 이번 문제에서는 이분 탐색의 시작과 끝을 나무를 자르는 높이의 시작(0)과 끝(max(list))으로 설정해서 진행했음
2. 나는 이분 탐색은 인덱스를 활용해서 진행한다는 생각이 있었어서 리스트를 중간에 한번 생성했었는데, 이렇게 하느라 시간초과가 자꾸 떴었음
"""